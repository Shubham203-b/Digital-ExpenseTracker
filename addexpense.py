...
class AddExpense(Screen):
    ...
    def add_expense(self):
        ...
        # Budget check
        try:
            from budget import Budget, check_budget_and_alert
            Budget.ensure_table()
            from datetime import datetime as dt
            q_date = dt.strptime(exp_date, '%Y-%m-%d')
            totals = Expenses.get_totals(q_date)
            currency = self.app.config.get('CustSettings', 'Currency')
            for period in ('day', 'month', 'year'):
                limit = Budget.get_budget(period)
                if limit > 0:
                    check_budget_and_alert(period.capitalize(), totals[period], limit, currency)
        except Exception:
            pass
